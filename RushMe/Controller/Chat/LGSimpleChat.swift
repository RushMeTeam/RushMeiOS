...
class LGChatMessageCell : UITableViewCell {
  ...
  lazy var opponentImageView: UIImageView = {
    ...
    let halfHeight = opponentImageView.bounds.width / 2.0
  ...
  private let minimumHeight: CGFloat = 32.0 // arbitrary minimum height
  ...
  private func styleTextViewForSentBy(sentBy: LGChatMessage.SentBy) {
    let halfTextViewWidth = self.textView.frame.width / 2.0
    let targetX = halfTextViewWidth + padding
    let halfTextViewHeight = self.textView.frame.height / 2.0
    switch sentBy {
    case .Opponent:
...
class LGChatController : UIViewController, UITableViewDelegate, UITableViewDataSource, LGChatInputDelegate {
  ...
  private func listenForKeyboardChanges() {
    ...
    defaultCenter.addObserver(self, selector: #selector(LGChatController.keyboardWillChangeFrame(note:)), name: NSNotification.Name.UIKeyboardWillChangeFrame, object: nil)
  }
  ...
  @objc func keyboardWillChangeFrame(note: NSNotification) {
    ...
    if self.bottomChatInputConstraint.constant != 0 {
     self.bottomChatInputConstraint.constant = 0 
    }
    else {
      if #available(iOS 11.0, *) {
        self.bottomChatInputConstraint.constant = -(keyboardFrame.size.height - self.view.safeAreaInsets.bottom)
      } else {
        self.bottomChatInputConstraint.constant = -(keyboardFrame.size.height)
      }
    }
  ...
  private func scrollToBottom() {
    if messages.count > 0 {
      ...
      let lastIndexPath = NSIndexPath(row: lastItemIdx, section: Constants.MessagesSection)
  ...
  func chatInput(chatInput: LGChatInput, didSendMessage message: String) {
    let newMessage = LGChatMessage(content: message, sentByString: "LGChatMessageSentByUser")
...
class LGChatInput : UIView, LGStretchyTextViewDelegate {
  ...
  func setupSendButton() {
    ...
    self.sendButton.setTitleColor(RMColor.AppColor, for: .normal)
    self.sendButton.addTarget(self, action: #selector(LGChatInput.sendButtonPressed), for: .touchUpInside)
    self.sendButton.bounds = CGRect(x: 0, y: 0, width: 40, height: 64)
  ...
  func setupTextViewConstraints() {
    ...
    heightConstraint = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .height, multiplier: 1.00, constant: 128)
...
class LGStretchyTextView : UITextView, UITextViewDelegate {
  ...
  func setup() {
    ...
    textContainerInset = UIEdgeInsets(top: 5, left: 6, bottom: 6, right: 6)
    delegate = self
  }
